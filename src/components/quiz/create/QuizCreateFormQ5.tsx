...
const QuizCreateFormQ5 = () => {
  ...
  const [question, setLocalQuestion] = useState(questions[4].question);
  const [answer, setLocalAnswer] = useState<'O' | 'X' | null>(
    questions[4].answer === true ? 'O' : questions[4].answer === false ? 'X' : null
  );

  const handleQuestionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.value.length <= 25) {
      setLocalQuestion(e.target.value);
      setQuestion(4, e.target.value);
    }
  };

  const handleAnswerSelect = (selected: 'O' | 'X') => {
    setLocalAnswer(selected);
    setAnswer(4, selected === 'O');
  };
